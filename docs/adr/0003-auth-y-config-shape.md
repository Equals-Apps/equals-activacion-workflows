...
- **Shape de workflows por-entidad desde el día uno.** Cada entrada del array `WORKFLOWS` (`lib/workflows.ts`) trae su propia env var de whitelist (`env.allowedEmails`), aunque hoy exista una sola entidad. Sumar una entidad nueva después es agregar un objeto + env vars nuevas, no rediseñar el tipo — que es justo lo que ADR-0001 pide no bloquear.
